...
def handle_data(context, data):
    ...
    # 计算现在的总资产，以分配资金，这里是等额权重分配
        ...
        previousdate = str(context.previous_date)[0:10]
        # 获得因子排序
        a,b=getRankedFactors(g.factors,todayStr,previousdate)
        # 计算每个股票的得分
...
def getRankedFactors(f,d,previousdate):
    # 获得股票的基本面数据，这个API里面有，g.q是一个全局通用的查询
    
    far = get_factor_values(g.all_stocks,factors=f, end_date=previousdate, count=1)
    first_factor_name=f[0]
    for i in f:
        if i==first_factor_name:
            df=far[i].T
            df.columns=[first_factor_name]
        else:
            df1=far[i].T        
            df1.columns=[i]
            df=df.join(df1)
