...
# # ETF轮动策略分析：多类别配置，低回撤设计
# ## 策略概述
# 该策略是一个多类别ETF轮动策略，通过在不同资产类别中选择表现最佳的ETF进行配置，实现风险分散和收益优化。策略覆盖A股指数、全球股指、国内期货、全球期货和REITs等多个资产类别，旨在通过跨类别轮动降低整体回撤风险。

# ## 核心逻辑
# ### 1. 资产类别划分
# 策略将ETF分为5个类别：

# - A股指数 ：沪深300ETF、创业板ETF
# - 全球股指 ：纳斯达克ETF、印度基金、德国ETF、日经ETF
# - 国内期货 ：有色期货ETF、能源化工ETF、豆粕ETF
# - 全球期货 ：白银基金、黄金ETF、南方原油
# - REITs ：蛇口产业园、盐田港REITs、绿能REITs等
# ### 2. 筛选条件
# 策略使用两个核心指标筛选ETF：

# - 涨幅 ：计算13个交易日的涨幅
# - 均线状态 ：当前价格与10日均线的差值（正数表示价格在均线上方）
# ### 3. 轮动机制
# - 每日计算 ：每天21:00计算各ETF的涨幅和均线状态
# - 类别筛选 ：每个类别选择涨幅>0且均线状态>0的ETF
# - 类别轮动 ：每个类别选择表现最好的ETF（涨幅最高）
# - 资金分配 ：等权重分配，每个类别分配总资金的1/5
# ### 4. 交易执行
# - 卖出操作 ：卖出不在目标列表中的ETF
# - 买入操作 ：买入目标列表中的ETF，按计算的股数执行
# - 交易时间 ：次日9:32执行交易
