...
## 策略概述
# 该策略是一个结合动量轮动、RSRS择时和卡尔曼滤波技术的ETF投资策略。通过卡尔曼滤波平滑价格数据，计算动量指标进行ETF排序，再结合RSRS择时信号决定买入、持有或卖出，旨在提高策略的稳定性和收益。

# ## 核心逻辑
# ### 1. 资产池构建
# - ETF池 ：包含上证50ETF、中证消费ETF、沪深300ETF、创业板50ETF
# - 备选池 ：注释中提到用流动性和市值更大的50ETF分别代替宽指ETF
# - 目标持仓 ：每次只持有评分最高的1只ETF（ g.stock_num = 1 ）
# ### 2. 动量计算与排序（卡尔曼滤波增强）
# - 数据处理 ：使用卡尔曼滤波平滑ETF价格数据，减少噪声影响
# - 动量计算 ：对平滑后的价格序列进行线性回归，取斜率作为动量指标
# - 排序逻辑 ：按动量指标降序排序，选择排名第一的ETF
# ### 3. RSRS择时信号
# - RSRS指标 ：基于沪深300指数的高低点线性回归斜率
# - 计算方法 ：
#   1. 计算最近18天的高低点回归斜率
#   2. 计算最近600天斜率的标准分（z-score）
#   3. 结合回归拟合度（R²）计算最终RSRS得分
# - 信号生成 ：
#   - RSRS得分 > 0.7：BUY信号
#   - RSRS得分 < -0.7：SELL信号
#   - 其他情况：KEEP信号
...
#   3. 根据信号执行操作：
#      - SELL信号：卖出所有持仓
#      - BUY或KEEP信号：调整持仓至目标ETF
# ### 5. 风险控制
# - 止损机制 ：当持仓ETF亏损90%时触发止损（实际意义不大，因为ETF几乎不可能亏损90%）
