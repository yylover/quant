# 克隆自聚宽文章：https://www.joinquant.com/post/37161
# 标题：随机森林 - 多因子模型
# 作者：QUX_1369

# 克隆自聚宽文章：https://www.joinquant.com/post/33840
# 标题：机器学习策略分享
# 作者：leo713
...
def backtest(backtest_dt, timer):
    ...
    if g.model is None:
        print('更新因子权重')
        dates = get_trade_days(end_date=backtest_dt, count=300)[::30]
        ...
        g.model = RandomForestClassifier(n_estimators=50, criterion='gini',
    ...
    current_df['label'] = g.model.predict(current_df[features])
    stock_to_buy = current_df[current_df.label == 1].sort_values('score').tail(g.stock_num)
    stock_to_buy = current_df.sort_values('score').tail(g.stock_num)
    stock_to_buy.loc[:, 'time'] = backtest_dt
    return stock_to_buy

def market_open(context):
    if g.timer % g.days_tc == 0:
