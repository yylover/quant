...
策略思路：
--------
1. 使用LightGBM分类模型预测股票涨跌方向
2. 预测结果分为三类：买入（1）、持有（0）、卖出（-1）
3. 选取预测为买入且概率最高的N只股票进行投资
4. 配合RSRS择时信号控制仓位
5. 等权重分散持仓，降低风险

模型特点：
---------
- LightGBM：轻量级梯度提升框架，训练速度快
- 分类目标：预测未来20日涨跌方向
- 特征工程：结合基本面、技术面、市场情绪
- 模型更新：每月重新训练一次
...
依赖库：lightgbm, numpy, pandas, statsmodels
"""
...
def set_parameter(context):
    ...
    g.buy_threshold = 5: 买入阈值（%）
        - 未来20日收益率 > 5% 分类为买入
        - 适当的阈值避免频繁交易
    
    g.sell_threshold = -5: 卖出阈值（%）
        - 未来20日收益率 < -5% 分类为卖出
        - 避免持有下跌股票
    
    g.prob_threshold = 0.6: 概率阈值
        - 预测为买入的概率 > 0.6 才考虑买入
        - 确保预测的可靠性
...
def market_open(context):
    """
    开盘时运行函数，每日执行一次
    核心逻辑：RSRS择时 + LightGBM选股交易
    """
    # 首次运行：预热RSRS数据并训练模型
    if g.days == 1:
        warm_up_rsrs(context)
        train_lightgbm_model(context)
        g.init = False
        return
...
def train_lightgbm_model(context):
    ...
    try:
        ...
        log.info(f'LightGBM模型训练完成，样本数：{len(df)}，特征数：{len(g.feature_names)}')
        log.info(f'分类分布：买入({class_counts.get(1, 0)}), 持有({class_counts.get(0, 0)}), 卖出({class_counts.get(-1, 0)})')
        
    except Exception as e:
        log.error(f'LightGBM模型训练失败：{str(e)}')
...
def calculate_features(context, stock_pool):
    ...
    try:
        ...
        features['rsi'] = 50
        features['bb_position'] = 0.5  # 布林带位置
        features['volatility_20d'] = 0
        features['volume_ma_ratio'] = 1  # 成交量均线比
        ...
        for stock in features.index[:500]:
            try:
                prices = get_price(stock, end_date=context.current_dt, 
                                   frequency='daily', fields=['close', 'volume', 'money', 'high', 'low'], 
                ...
                if len(sma) >= 20:
                    ...
                    features.loc[stock, 'bb_position'] = (current_price - lower_band.iloc[-1]) / bb_range if bb_range > 0 else 0.5
                ...
                # 成交量均线比
                if len(prices) >= 20:
                    features.loc[stock, 'volume_ma_ratio'] = prices['volume'].iloc[-1] / prices['volume'].tail(20).mean()
...
def calculate_labels(context, stock_pool):
    """计算标签（涨跌方向：买入/持有/卖出）"""
    try:
        labels = pd.DataFrame(index=stock_pool, columns=['direction'])
        
        # 获取基准指数（沪深300）的收益率
        benchmark_prices = get_price('000300.XSHG', end_date=context.current_dt, 
                                      frequency='daily', fields=['close'], 
                                      count=g.prediction_days + 10)
        if benchmark_prices is not None and len(benchmark_prices) >= g.prediction_days:
        ...
        for stock in stock_pool[:500]:
            try:
                ...
                if future_prices is None or len(future_prices) < g.prediction_days:
                    labels.loc[stock, 'direction'] = 0
                    continue
                
                # 计算未来20日收益率
                current_price = future_prices['close'].iloc[0]
                future_price = future_prices['close'].iloc[-1]
                stock_return = (future_price / current_price - 1) * 100
                
                # 相对收益率（vs基准）
                relative_return = stock_return - (benchmark_return * 100)
                
                # 分类
                if stock_return > g.buy_threshold:
                    labels.loc[stock, 'direction'] = 1  # 买入
                elif stock_return < g.sell_threshold:
                    labels.loc[stock, 'direction'] = -1  # 卖出
                else:
                    labels.loc[stock, 'direction'] = 0  # 持有
...
def lightgbm_trade(context):
    ...
    try:
        ...
        proba = g.model.predict_proba(features_df)
        
        features_df['direction'] = predictions
        
        # 获取买入类别的概率（类别1）
        # LightGBM的类别顺序可能是-1, 0, 1或其他顺序，需要确认
        # 这里假设是按数值排序：-1, 0, 1
        if g.model.n_classes_ == 3:
            # 找到类别1在proba中的索引
            class_1_idx = 2  # 假设顺序是-1, 0, 1
            features_df['prob_buy'] = proba[:, class_1_idx]
        else:
            features_df['prob_buy'] = 0
        
        # 筛选预测为买入且概率高的股票
        buy_candidates = features_df[
            (features_df['direction'] == 1) & 
            (features_df['prob_buy'] >= g.prob_threshold)
        ]
        
        if len(buy_candidates) == 0:
            # 如果没有符合条件的股票，降低概率阈值
            buy_candidates = features_df[features_df['direction'] == 1]
            if len(buy_candidates) == 0:
                return
        
        # 按买入概率排序，选择前N只
        buy_candidates = buy_candidates.sort_values('prob_buy', ascending=False)
        selected_stocks = buy_candidates.head(g.stock_num).index.tolist()
        ...
        log.info(f'LightGBM选股完成，选股数：{len(selected_stocks)}，买入概率范围：{buy_candidates["prob_buy"].min():.2f} ~ {buy_candidates["prob_buy"].max():.2f}')
        
    except Exception as e:
        log.error(f'LightGBM交易失败：{str(e)}')
