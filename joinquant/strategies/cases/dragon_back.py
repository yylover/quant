...
下面是对当前仓库里 **`strategies/cases/dragon_back.py`**（聚宽帖克隆：龙回头 / EragonV5 思路）的**交易策略分析**，按「数据流 → 时间轴 → 买卖规则 → 风险点」整理。

---

## 1. 策略是什么（定位）

- **来源**：聚宽文章「复制+改进，龙回头策略…」，在 **JQ 龙回头 V5** 上做了 **Eragon 量价、均线踩线** 等过滤。  
- **标的**：**A 股股票**（非 ETF），偏 **短线题材 + 连板后回调**，配合 **龙虎榜 + 小市值**。  
- **持仓规模**：代码里 **`g.stocknum = 2`**，资金上通过 **`available_cash == total_value` 时现金对半分** 等方式，实质倾向 **最多 2 只** 重仓。  
- **基准**：**中证 1000**（`000852.XSHG`），并开启 **`avoid_future_data`**。
...
| 文件 | 谁写 | 内容含义 |
|------|------|----------|
| **`Crouch_eye.csv`** | `after_close_eye`（21:00） | **「眼」**：龙虎榜 ∩ 小市值 等初筛结果，带 `riselevel`（来自 `is_falls` 比值或 0=剔除）。 |
| **`Crouch_brain.csv`** | `after_close_brain`（22:00） | **「脑」**：基于 eye 的 **买/卖信号**（`buy_XXX` / `sell`），供次日开盘读。 |
| **`Crouch_log.csv`** | 各交易函数 | 运行日志。 |

逻辑顺序：**当晚 eye 记录观察名单 → 当晚 brain 算是否出现「龙回头」结构 + 决策树 → 次日开盘按 brain 执行**。
...
| 时间 | 函数 | 作用摘要 |
|------|------|----------|
| **`open`** | `market_open` | 读 **brain** 里 **`date == lastd_date`**（上一交易日）的信号；**先卖后买**；买价需在 **`[0.982, 1.05] × 信号里的参考价`** 附近。 |
| **13:58** | `market_run` | 持仓：**近跌停放量**、**亏损<-10%**、**盈利>200%（`ret>2`）** 等则市价清仓。 |
| **14:55** | `market_close` | 类似风控 + **持有天数/收益失速** 尾盘清；可按 **开盘价区间** 补买未持仓标的。 |
| **20:00** | `after_market_close` | 打汇总日志。 |
| **21:00** | `after_close_eye` | 更新 **eye**（龙虎榜、市值、停牌、去重等）。 |
| **22:00** | `after_close_brain` | 更新 **brain**（两连板 + 回调 + 均线 + 多规则买；持仓卖信）。 |

---

## 4. 「眼」`after_close_eye`（粗筛）

- 全市场股票 → 去停牌、ST、退市字样、上市 **>100 天**。  
- **市值**：`market_cap < 500`（亿级口径以聚宽字段为准），并按 **流通市值升序** 取一批。  
- 与 **当日龙虎榜**（`get_billboard_list`，`count=1`）取交集。  
- **`is_falls`**：过滤 **近 60 日涨幅过猛**（创业板/科创板阈值更严），否则返回 **`close_last/close_min` 作 `riselevel`**。  
- 近 **20 日**有停牌则剔除；**近 3 天 eye 已出现** 的代码不重复写入。

---

## 5. 「脑」`after_close_brain`（核心 Alpha）

在 **eye 近 30 天**、且 **brain 近 7 天未重复买** 的前提下，对每条历史信号 roughly：

1. **两连板**：从 `Sign_date` 起连续两段交易日，**收盘价均触及涨停价**（`close` vs `high_limit`）。  
2. **回调结构**：从 **`T1` 到今日**，阶段最高价相对 **T 日收盘价** 不超过约 **+20%**；阶段最低价相对阶段最高价有足够 **回撤（<0.8×阶段高）**；在 **最低点** 处价格 **跌破 MA5/MA10**（回踩）。  
3. **再启动信号**：从低点日后逐日扫，直到出现 **收盘站上 MA5 且站上 MA10**，或 **自当日低点反弹幅度 >8%**，记 **`prebuy`**。  
4. **触发写入 brain 的买信号**：要求 **`prebuy` 且 `sink_date == today`**（即 **信号在「今天收盘」才确认**），再算 PE/PB、CYF、换手分段、**中证 1000 昨日涨跌 `index_rise`** 等。  
5. **多组规则**（`buy_BL`、`buy_ML`、`buy_DG`、`buy_BC`、`buy_JZ`、`buy_ZZ`、`buy_HT`、`buy_XD`）：不同组合约束 **`dur_MinD`、`dur_TD`、`pop`、换手比、`up_level`** 等，命中则 **`write_file` 到 `Crouch_brain.csv`**。

对 **已有持仓** 另写 **sell**（止损、超盈、短期弱势、失速、跌停、天量等），供 **次日开盘** 读入卖出。

---

## 6. 开盘 `market_open`（执行）

- **`lastd_date = get_trade_days(end_date=today, count=2)[0]`** → 在 **T 日早盘**，这是 **T-1（上一交易日）**。  
- 读 **brain 里 `date == T-1` 的行**：即 **昨晚（T-1 日 22:00）写好的信号**，在 **T 日开盘** 使用 → **时间上不偷看 T 日收盘**，与「隔日执行」一致。  
- **卖**：`ret > 2`（**+200%**）止盈；或代码在 **`g.df_sell`** 里则清仓。  
- **买**：现价在 **`lastd_price` 的 98.2%～105%** 内则 **`order_target_value`**；空仓时常 **半仓一只** 的现金分配逻辑。

---

## 7. 重要细节与风险（分析用）

1. **`risk_bench`**：函数在 **`return index_rise` 之后** 还有一大段「跌停家数风控」**永远不会执行**（死代码）；实际进决策树的 **`index_rise`** 只是 **中证1000 指数昨日涨跌幅**。  
2. **`g.df_buy.sort_values(['data'], ...)`**：列名是否为 **`data`** 需与 CSV 一致，若实际是 **`人气` 等** 可能报错或需核对聚宽导出列名。  
3. **`ret > 2`**：止盈阈值是 **+200%**，非常极端，多数情况靠 **brain 卖信** 或其它条件离场。  
4. **`future_price`**：注释写明用 **未来价格** 做测试，**若接入主逻辑会造成严重前视**；当前应 **仅离线分析用**，勿与实盘信号混用。  
5. **文件头 `"""` / `"""` 空块**：第 5–8 行 docstring 结构不完整，不影响后面 `from jqdata`，但建议日后整理。  
6. **实盘**：涨停、跌停、排队、龙虎榜股票流动性等，回测通常偏乐观。

---

## 8. 一句话总结

**`dragon_back.py` = 龙虎榜 + 小市值「眼」选种 →「脑」在连板后回调结构里找 **回踩均线/反弹** 的 **prebuy**，且 **仅当信号在当日收盘确认** 才写入 brain → **次日开盘** 按 CSV **先卖后买**，盘中与尾盘继续 **止损/止盈/情绪** 管理；属于 **事件驱动 + 技术形态 + 多条件决策树** 的短线股票策略，**依赖聚宽 `read_file/write_file` 与 CSV 初始存在**。

若你需要，我可以再单独列一张 **「brain 里各 `buy_XX` 触发的文字条件对照表」**（只基于已读代码，不跑回测）。
"""
# 导入函数库
from jqdata import *
