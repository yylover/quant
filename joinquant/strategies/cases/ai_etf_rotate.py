...
在 **约 55 只 ETF**（商品、美股、国际、港股、A 股大盘/行业、债券、货币）里做 **轮动**：  
**进攻** = 5 日动量 + MACD 金叉/柱线 + 分类强度等多因子打分；  
**防守** = 连续多日没有合格进攻标的时，才配置 **债券/货币 ETF**。  
同时配合 **波动率（用 50ETF 实现的“伪 VIX”）** 调节单只上限仓位，以及 **止损 / 跟踪止盈 / MACD 死叉卖出** 和 **盘中多次风控检查**。

基准：**510500（中证 500ETF）**（`g.config['benchmark']`）。

---

## 2. 池子与分类

- **`g.etf_pool`**：55 只左右，覆盖 8 大类（商品、美股、国际、港股、A 股大盘、A 股行业、债券、货币）。  
- **`g.etf_categories`**：每类对应一串代码；**`g.category_map`** 做「代码 → 类别」映射。  
- **进攻池**：除 **bond、cash** 外的 ETF。  
- **防御池**：`bond` + `cash` 列表。
...
| 时间 | 函数 | 作用 |
|------|------|------|
| **09:30** | `market_open` | 主逻辑：先算指标与分类强度 → **先卖后买**。 |
| **09:30** | `prepare` | 空函数 `pass`，无实际作用。 |
| **10:30 / 11:30 / 14:00 / 14:50** | `check_stop_loss` | 仅做 **-6% 止损** 与 **盈利超 15% 后从最高价回撤 10% 止盈**（与 `g.config` 一致）。 |

说明：开盘集中逻辑在 **`market_open`**；盘中四次只重复 **止损/跟踪止盈**，**不再**在 `check_stop_loss` 里处理 MACD 死叉（死叉在 `execute_sell_etf` 里）。

---

## 4. 开盘前模块（`market_open` 内部顺序）

1. **`update_vix_and_position`**  
   - 用 **510050** 近 20 日收盘价算对数收益波动率，**年化 ×100** 当作「VIX 代理」。  
   - 高于阈值则把 **`max_single_position`** 调到 **25% / 30% / 35%**（默认阈值 18，代码中 0.7× 为中档）。

2. **`update_category_strength`**  
   - 用约 **20 日** 窗口拉全池价格，**按 ETF 算**「平均日收益 + 区间总收益」的加权得分，再 **按类取平均** 得到 **category strength**。  
   - 取 **Top 3 类** 存入 **`g.top_categories`**，供后续 **`get_category_factor`**（强势类 **1.3**，否则 **1.0**）给资金分配加权。

3. **`calculate_indicators_etf`**  
   - 用 **talib** 对每只 ETF 算 **MACD、RSI、MA20、5 日均量**，写入 **`g.macd_data` / `g.ma20_data` / `g.volume_data`**。  
   - 数据来自 **90 日** 日线（用于 MACD 足够长）。

4. **`execute_sell_etf`**（先卖）  
   - **固定止损**：浮亏 ≤ **-6%**。  
   - **跟踪止盈**：历史最高盈利曾 **> 15%**，且从持仓期最高价 **回撤 > 10%**（用 `g.max_prices` 记录高点）。  
   - **进攻品种**（非 bond/cash）：**MACD 柱由正转负** 或 **DIF 下穿 DEA（死叉）** 则卖。  
   - 债券/货币类 **不做 MACD 死叉这一条**（代码里 `category not in ['bond','cash']`）。

5. **`execute_buy_etf`**（后买）  
   - 先 **`get_offensive_buy_list`**；若有候选，则 **`buy_offensive_etfs`**，并把 **`g.no_offensive_count` 清零**。  
   - 若 **没有** 进攻候选：计数 **+1**；仅当 **连续 ≥3 天** 无进攻、且当前 **没有** 进攻仓、且防御仓 **未满** 时，才 **`buy_defensive_etfs`**。

---

## 5. 进攻买入逻辑（核心）

**`get_offensive_buy_list`** 大致步骤：

1. **排除** 债券/货币；若开启 **`enable_category_limit`**，每类已有持仓数 ≤ **`max_per_category`（1）** 才继续筛。  
2. **过滤**：RSI **≤ 70**；收盘价 **> MA20** 且 **MA20 趋势向上**（`ma20[-1] > ma20[-5]`）。  
3. 算 **5 日收益率**，全市场进攻 ETF **按 5 日收益排序**，只取 **Top 15** 进入打分。  
4. **打分（文档 100 分制）**：  
   - 动量 5 日：约 **0～4% 映射到 0～60 分**；  
   - 金叉（DIF 上穿 DEA 且柱为正）**+20**；仅 DIF>DEA 未金叉 **+10**；  
   - MACD 柱强度 **最多 10 分**；  
   - 分类因子：若属 **Top3 强势类**，`get_category_factor` 为 **1.3**，再映射到加分（约 10 分档）。  
5. **最终买入条件**：**总分 > 50** 且 **（金叉 或 5 日收益 > 3%）**。

**`buy_offensive_etfs`**：在候选列表里按分数排序，受 **最多 `hold_max`（3）只**、**单只不超过 `max_single_position`×净值**、现金分配等约束，用 **`order_target_value`** 下单。
...
- **连续 3 天**没有进攻候选（`g.no_offensive_count >= 3`）；  
- 当前持仓里 **没有** 进攻类（非 bond/cash）；  
- 防御持仓 **少于 `defensive_max_hold`（2）**。

**`rank_defensive_etfs`**：在债券+货币里按 **约 20 日收益** 排序，且收益 ≥ **`defensive_min_return`（-0.5，即 -50% 量级配置里写的是百分比*100 的对比，见代码 704 行）** 的候选（实现上是 `total_return >= defensive_min_return * 100`，即 **-50%** 阈值极宽，实际等于几乎不砍）。

---

## 7. 与文档/注释的差异（便于核对）

- 模块 docstring 里的 **回测收益数字** 仅为示例，**换区间会变**。  
- **`should_sell_etf`** 里注释写「盈利>10%后回撤15%」，与 **`g.config` 的 15%/10%** 不一致，以 **`trail_profit_trigger` / `trail_stop_drawdown`** 为准。  
- **`set_order_cost`** 使用 **卖出印花税 0.1%** 的「股票类」模板；**ETF 实盘**通常无印花税，回测可能 **偏悲观** 或需按平台改。
...
| 方面 | 说明 |
|------|------|
| **优点** | 多资产分散、强弱分类 + 动量 + 技术过滤、无进攻时转债券/货币，逻辑完整；风控（止损、止盈、死叉）清晰。 |
| **注意** | 参数多（阈值、Top15、评分权重），**过拟合风险**需样本外验证；**9:30** 使用日线指标时，需确认聚宽回测里「当日 bar」是否含未收盘数据（通常日频用昨收为主）。 |
| **依赖** | `from kuanke.wizard import *` 需在聚宽环境可用；`talib` 需支持。 |
