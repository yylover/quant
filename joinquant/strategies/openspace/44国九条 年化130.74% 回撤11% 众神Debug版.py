...
# 策略收益 146.02%   基准收益 -16.47%   Alpha 0.38   Beta 0.53   Sharpe 1.46   最大回撤  15.93%
...
def initialize(context):
    ...
    run_daily(trade_afternoon, time='14:00', reference_security='399101.XSHE')  # 每天14:00检查涨停股
    run_daily(stop_loss, time='10:00')  # 每天10:00执行止损
    run_daily(close_account, '14:50')  # 每天14:50执行特殊月份清仓
    run_weekly(weekly_adjustment, 2, '10:00')  # 每周二10:00调仓
    #run_weekly(print_position_info, 5, time='15:10', reference_security='000300.XSHG')  # 每周五打印持仓信息
...
def get_stock_list(context):
    ...
    q = query(
        ...
        income.np_parent_company_owners > 0,   # 归属于母公司所有者的净利润(元)大于0
        income.net_profit > 0,  # 净利润(元)大于0
        income.operating_revenue > 1e8,  # 营业收入 (元)大于1亿
        indicator.roe > 0,  # 净资产收益率大于0
        indicator.roa > 0,  # 资产收益率大于0
    ).order_by(valuation.market_cap.asc()).limit(g.stock_num * 3)  # 按市值从小到大排序，取前3倍持股数量的股票
...
def check_remain_amount(context):
    """检查剩余资金，如有空仓则买入股票或货币基金"""
    addstock_num = len(g.other_sale)  # 其他原因卖出的股票数量
    loss_num = len(g.stoploss_list)  # 止损卖出的股票数量
    empty_num = addstock_num + loss_num  # 空仓数量
...
def trade_afternoon(context):
    ...
    if g.trading_signal:
        ...
        check_remain_amount(context)  # 检查剩余资金
        buy_security(context, [g.money_etf], 1)  # 买入货币基金
        
#1-7 止盈止损
def stop_loss(context):
    """止盈止损函数，执行个股止盈止损和市场趋势止损"""
    if g.run_stoploss:
...
def filter_stocks(context, stock_list):
    """过滤股票，排除停牌、ST、退市、创业板、科创板、北交所、涨跌停、次新股等"""
    ...
    for stock in stock_list:
        ...
        if '退' in current_data[stock].name:  # 退市
            continue
        if stock.startswith('30') or stock.startswith('68') or stock.startswith('8') or stock.startswith('4'):  # 市场类型（创业板、科创板、北交所）
            continue
