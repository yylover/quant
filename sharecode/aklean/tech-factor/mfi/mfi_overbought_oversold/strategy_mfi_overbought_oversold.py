...
class MFIOversoldOverboughtStrategy:
    def __init__(self):
        self.data = None
        self.results = {}
    ...
    def generate_signals(self):
        ...
        # 买入信号：MFI < 20（超卖）
        buy_condition = data['mfi'] < 20
        
        # 卖出信号：MFI > 80（超买）
        sell_condition = data['mfi'] > 80
    ...
    def plot_results(self):
        ...
        ax1.set_title('MFI超买超卖策略 - 沪深300', fontsize=16)
        ...
        # 添加MFI阈值线
        ax2.axhline(y=20, color='green', linestyle='--', label='超卖阈值(20)')
        ax2.axhline(y=80, color='red', linestyle='--', label='超买阈值(80)')
        ...
        plt.tight_layout()
        plt.savefig('mfi_overbought_oversold_strategy_backtest.png', dpi=300, bbox_inches='tight')
    
    def save_results(self):
        """保存回测结果"""
        # 保存交易记录
        trades = self.data[self.data['signal'] != 0].copy()
        trades.to_csv('mfi_overbought_oversold_strategy_backtest.csv')
    
    def generate_report(self):
        """生成策略说明文档"""
        report = f"""# MFI超买超卖策略说明
...
"""
        ...
        with open('mfi_overbought_oversold_strategy说明.md', 'w', encoding='utf-8') as f:
            f.write(report)
...
if __name__ == '__main__':
    strategy = MFIOversoldOverboughtStrategy()
    strategy.run()
