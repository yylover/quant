...
class OBVTrendStrategy:
    ...
    def __init__(self, data, obv_ma_period=20, price_ma_period=50):
        """
        初始化OBV趋势跟踪策略
        
        参数:
            data: 包含open, high, low, close, volume列的DataFrame
            obv_ma_period: OBV均线周期
            price_ma_period: 价格均线周期
        """
        self.data = data.copy()
        self.obv_ma_period = obv_ma_period
        self.price_ma_period = price_ma_period
        self.output_dir = "/Users/quickyang/Documents/workspace/develop/mycode/quant/sharecode/aklean/tech-factor/obv/obv_trend"
    ...
    def calculate_obv(self):
        ...
        # 计算OBV均线
        self.data['OBV_ma'] = self.data['OBV'].rolling(window=self.obv_ma_period).mean()
        
        # 计算价格均线
        self.data['price_ma'] = self.data['close'].rolling(window=self.price_ma_period).mean()
        
        # 计算价格是否在均线上方
        self.data['price_above_ma'] = self.data['close'] > self.data['price_ma']
    ...
    def generate_signals(self):
        ...
        buy_condition = (self.data['price_above_ma']) & \
        ...
        sell_condition = (~self.data['price_above_ma']) | \
                       ((self.data['OBV'] < self.data['OBV_ma']) & \
                        (self.data['OBV'].shift(1) >= self.data['OBV_ma'].shift(1)))
    ...
    def calculate_metrics(self):
        ...
        metrics = {
            ...
            '参数': f"obv_ma_period={self.obv_ma_period}, price_ma_period={self.price_ma_period}"
        }
    ...
    def plot_backtest(self):
        ...
        plt.plot(self.data.index, self.data['price_ma'], label=f'{self.price_ma_period}日均线', color='red', linewidth=2)
        ...
        plt.plot(self.data.index, self.data['OBV_ma'], label=f'{self.obv_ma_period}日OBV均线', color='orange', linewidth=2)
        ...
        plt.tight_layout()
        plt.savefig(f"{self.output_dir}/obv_trend_strategy_backtest.png", dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def run_strategy(self):
        ...
        # 保存回测数据
        self.data.to_csv(f"{self.output_dir}/obv_trend_strategy_backtest.csv")
        
        # 保存策略说明
        with open(f"{self.output_dir}/obv_trend_strategy说明.md", 'w', encoding='utf-8') as f:
            f.write(f"""# OBV趋势跟踪策略说明
...
## 参数设置
- OBV均线周期: {self.obv_ma_period}
- 价格均线周期: {self.price_ma_period}
