...
class BollBandWidthStrategy:
    ...
    def __init__(self, data, period=20, multiplier=2, width_period=20):
        """
        初始化布林带宽度策略
        ...
        参数:
            ...
            period: 布林线计算周期
            multiplier: 标准差倍数
            width_period: 布林带宽度计算周期
        """
        self.data = data.copy()
        self.period = period
        self.multiplier = multiplier
        self.width_period = width_period
        self.output_dir = "/Users/quickyang/Documents/workspace/develop/mycode/quant/sharecode/aklean/tech-factor/boll/boll_band_width"
    ...
    def calculate_bollinger_bands(self):
        ...
        # 计算布林带宽度的移动平均
        self.data['band_width_ma'] = self.data['band_width'].rolling(window=self.width_period).mean()
        
        # 计算宽度变化率
        self.data['width_change'] = self.data['band_width'] / self.data['band_width'].shift(1) - 1
    ...
    def generate_signals(self):
        ...
        # 计算宽度百分比变化
        self.data['width_pct_change'] = self.data['band_width'].pct_change() * 100
        
        # 买入信号：布林带宽度扩张超过5%，且价格突破上轨
        buy_condition = (self.data['width_pct_change'] > 5) & \
                      (self.data['close'] > self.data['upper_band'])
        
        # 卖出信号：布林带宽度收缩超过5%，且价格跌破下轨
        sell_condition = (self.data['width_pct_change']< -5) & \
    ...
    def calculate_metrics(self):
        ...
        metrics = {
            ...
            '参数': f"period={self.period}, multiplier={self.multiplier}, width_period={self.width_period}"
        }
    ...
    def plot_backtest(self):
        ...
        plt.plot(self.data.index, self.data['band_width_ma'], label='宽度移动平均', color='red', linewidth=2)
        ...
        plt.tight_layout()
        plt.savefig(f"{self.output_dir}/boll_band_width_strategy_backtest.png", dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def run_strategy(self):
        ...
        # 保存回测数据
        self.data.to_csv(f"{self.output_dir}/boll_band_width_strategy_backtest.csv")
        
        # 保存策略说明
        with open(f"{self.output_dir}/boll_band_width_strategy说明.md", 'w', encoding='utf-8') as f:
            f.write(f"""# 布林带宽度策略说明
