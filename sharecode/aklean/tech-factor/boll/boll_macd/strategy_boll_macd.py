...
class BollMACDStrategy:
    """布林线与MACD结合策略"""
    
    def __init__(self, data, period=20, multiplier=2, fast_period=12, slow_period=26, signal_period=9):
        """
        初始化布林线与MACD结合策略
        ...
        self.output_dir = "/Users/quickyang/Documents/workspace/develop/mycode/quant/sharecode/aklean/tech-factor/boll/boll_macd"
    ...
    def calculate_metrics(self):
        ...
        metrics = {
            ...
            '参数': f"period={self.period}, multiplier={self.multiplier}, fast_period={self.fast_period}, slow_period={self.slow_period}, signal_period={self.signal_period}"
        }
    ...
    def plot_backtest(self):
        ...
        plt.title('沪深300指数与布林线MACD策略交易信号')
        plt.legend()
        plt.grid(True, alpha=0.3)
        
        # 子图2：MACD指标
        plt.subplot(3, 1, 2)
        plt.plot(self.data.index, self.data['MACD'], label='MACD', color='blue', linewidth=2)
        ...
        plt.tight_layout()
        plt.savefig(f"{self.output_dir}/boll_macd_strategy_backtest.png", dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def run_strategy(self):
        ...
        # 保存回测数据
        self.data.to_csv(f"{self.output_dir}/boll_macd_strategy_backtest.csv")
        
        # 保存策略说明
        with open(f"{self.output_dir}/boll_macd_strategy说明.md", 'w', encoding='utf-8') as f:
            f.write(f"""# 布林线MACD结合策略说明
