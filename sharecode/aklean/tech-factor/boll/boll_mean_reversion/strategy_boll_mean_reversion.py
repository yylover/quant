...
class BollMeanReversionStrategy:
    ...
    def __init__(self, data, period=20, multiplier=2):
        ...
        self.output_dir = "/Users/quickyang/Documents/workspace/develop/mycode/quant/sharecode/aklean/tech-factor/boll/boll_mean_reversion"
    ...
    def calculate_bollinger_bands(self):
        ...
        # 计算价格位置百分比
        self.data['band_percent'] = (self.data['close'] - self.data['lower_band']) / (self.data['upper_band'] - self.data['lower_band'])
    ...
    def plot_backtest(self):
        ...
        plt.title('沪深300指数与布林线均值回归策略交易信号')
        plt.legend()
        plt.grid(True, alpha=0.3)
        
        # 子图2：价格位置百分比
        plt.subplot(3, 1, 2)
        plt.plot(self.data.index, self.data['band_percent'] * 100, label='价格位置百分比', color='purple', linewidth=2)
        plt.axhline(y=0, color='gray', linestyle='-', alpha=0.5)
        plt.axhline(y=100, color='gray', linestyle='-', alpha=0.5)
        
        plt.title('价格在布林带中的位置')
        plt.legend()
        plt.grid(True, alpha=0.3)
        ...
        plt.tight_layout()
        plt.savefig(f"{self.output_dir}/boll_mean_reversion_strategy_backtest.png", dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def run_strategy(self):
        ...
        # 保存回测数据
        self.data.to_csv(f"{self.output_dir}/boll_mean_reversion_strategy_backtest.csv")
        
        # 保存策略说明
        with open(f"{self.output_dir}/boll_mean_reversion_strategy说明.md", 'w', encoding='utf-8') as f:
            f.write(f"""# 布林线均值回归策略说明
