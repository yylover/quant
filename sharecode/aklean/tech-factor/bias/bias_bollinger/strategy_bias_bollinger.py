...
class BiasBollingerStrategy:
    def __init__(self, data, short_period=6, bollinger_period=15, bollinger_std=1.5, buy_threshold=-1.5, sell_threshold=1.5, stop_loss=0.08, take_profit=0.20):
        self.data = data.copy()
        ...
        self.output_dir = "/Users/quickyang/Documents/workspace/develop/mycode/quant/sharecode/aklean/tech-factor/bias/bias_bollinger"
    ...
    def calculate_bollinger_bands(self):
        """计算布林带指标"""
        # 计算布林带中轨（移动平均线）
        self.data['bollinger_mid'] = self.data['close'].rolling(self.bollinger_period).mean()
        
        # 计算标准差
        self.data['bollinger_std'] = self.data['close'].rolling(self.bollinger_period).std()
        
        # 计算布林带上轨和下轨
        self.data['bollinger_upper'] = self.data['bollinger_mid'] + (self.bollinger_std * self.data['bollinger_std'])
        self.data['bollinger_lower'] = self.data['bollinger_mid'] - (self.bollinger_std * self.data['bollinger_std'])
        
        # 计算价格相对于布林带的位置
        self.data['bollinger_position'] = (self.data['close'] - self.data['bollinger_mid']) / self.data['bollinger_std']
        
        # 识别价格触及布林带边界
        self.data['touch_upper'] = self.data['high'] >= self.data['bollinger_upper']
        self.data['touch_lower'] = self.data['low']<= self.data['bollinger_lower']
    ...
    def plot_results(self):
        ...
        plt.plot(self.data.index, self.data['bollinger_mid'], label=f'{self.bollinger_period}日均线', color='orange', linewidth=1.2)
        plt.plot(self.data.index, self.data['bollinger_upper'], label='布林带上轨', color='green', linestyle='--', linewidth=1)
        plt.plot(self.data.index, self.data['bollinger_lower'], label='布林带下轨', color='red', linestyle='--', linewidth=1)
        plt.fill_between(self.data.index, self.data['bollinger_lower'], self.data['bollinger_upper'], 
                        color='gray', alpha=0.1, label='布林带通道')
        ...
        plt.subplot(5, 1, 3)
        plt.plot(self.data.index, self.data['bollinger_position'], label='价格相对布林带位置', color='brown', linewidth=1.5)
        plt.axhline(y=self.bollinger_std, color='green', linestyle='--', linewidth=1)
        plt.axhline(y=-self.bollinger_std, color='red', linestyle='--', linewidth=1)
        plt.title('价格相对布林带位置')
        plt.legend()
        plt.grid(True)
        
        plt.subplot(5, 1, 4)
        plt.plot(self.data.index, self.data['bollinger_std'], label='价格波动率', color='cyan', linewidth=1.5)
        plt.title('市场波动率')
        plt.legend()
        plt.grid(True)
        ...
        plt.tight_layout()
        plt.savefig(f"{self.output_dir}/bias_bollinger_strategy_backtest.png", dpi=300, bbox_inches='tight')
        plt.close()
...
if __name__ == "__main__":
    ...
    # 创建策略实例
    strategy = BiasBollingerStrategy(data, short_period=6, bollinger_period=20, bollinger_std=2.0, buy_threshold=-2.5, sell_threshold=3.5)
    ...
    # 统计布林带触碰情况
    touch_upper_count = strategy.data['touch_upper'].sum()
    touch_lower_count = strategy.data['touch_lower'].sum()
    print(f"\n布林带触碰统计：")
    print(f"触碰上轨次数: {touch_upper_count}")
    print(f"触碰下轨次数: {touch_lower_count}")
