...
class BiasRsiStrategy:
    def __init__(self, data, short_period=6, rsi_period=14, buy_threshold=-1.5, sell_threshold=1.5, rsi_oversold=30, rsi_overbought=70, stop_loss=0.08, take_profit=0.20):
        self.data = data.copy()
        ...
        self.output_dir = "/Users/quickyang/Documents/workspace/develop/mycode/quant/sharecode/aklean/tech-factor/bias/bias_rsi"
    ...
    def plot_results(self):
        ...
        plt.subplot(5, 1, 3)
        plt.plot(self.data.index, self.data['rsi'], label=f'RSI({self.rsi_period})', color='orange', linewidth=1.5)
        plt.axhline(y=self.rsi_oversold, color='green', linestyle='--', linewidth=1, label=f'超卖({self.rsi_oversold})')
        plt.axhline(y=self.rsi_overbought, color='red', linestyle='--', linewidth=1, label=f'超买({self.rsi_overbought})')
        plt.fill_between(self.data.index, self.rsi_oversold, self.rsi_overbought, 
                        color='gray', alpha=0.1, label='正常区间')
        plt.title('RSI指标')
        plt.legend()
        plt.grid(True)
        
        plt.subplot(5, 1, 4)
        plt.plot(self.data.index, self.data['rsi_oversold'] * 100, label='RSI超卖', color='green', alpha=0.3)
        plt.plot(self.data.index, self.data['rsi_overbought'] * 100, label='RSI超买', color='red', alpha=0.3)
        plt.title('RSI超买超卖区域')
        plt.legend()
        plt.grid(True)
        ...
        plt.tight_layout()
        plt.savefig(f"{self.output_dir}/bias_rsi_strategy_backtest.png", dpi=300, bbox_inches='tight')
        plt.close()
...
if __name__ == "__main__":
    ...
    # 创建策略实例
    strategy = BiasRsiStrategy(data, short_period=6, rsi_period=14, buy_threshold=-2.5, sell_threshold=3.5, rsi_oversold=30, rsi_overbought=70)
    ...
    # 统计RSI超买超卖情况
    rsi_oversold_count = strategy.data['rsi_oversold'].sum()
    rsi_overbought_count = strategy.data['rsi_overbought'].sum()
    print(f"\nRSI超买超卖统计：")
    print(f"RSI超卖次数: {rsi_oversold_count}")
    print(f"RSI超买次数: {rsi_overbought_count}")
