...
class BiasChannelStrategy:
    def __init__(self, data, short_period=6, channel_period=15, channel_std=1.5, stop_loss=0.05, take_profit=0.15):
        self.data = data.copy()
        ...
        self.output_dir = "/Users/quickyang/Documents/workspace/develop/mycode/quant/sharecode/aklean/tech-factor/bias/bias_channel"
    ...
    def calculate_channel(self):
        """计算BIAS通道"""
        # 计算BIAS的移动平均线（通道中轨）
        self.data['channel_mid'] = self.data['bias_short'].rolling(self.channel_period).mean()
        
        # 计算BIAS的标准差
        self.data['channel_std'] = self.data['bias_short'].rolling(self.channel_period).std()
        
        # 计算通道上轨和下轨
        self.data['channel_upper'] = self.data['channel_mid'] + (self.channel_std * self.data['channel_std'])
        self.data['channel_lower'] = self.data['channel_mid'] - (self.channel_std * self.data['channel_std'])
        
        # 识别BIAS突破通道
        self.data['break_upper'] = self.data['bias_short'] > self.data['channel_upper']
        self.data['break_lower'] = self.data['bias_short']< self.data['channel_lower']
        
        # 计算BIAS相对于通道的位置
        self.data['bias_position'] = (self.data['bias_short'] - self.data['channel_mid']) / self.data['channel_std']
    ...
    def generate_signals(self):
        ...
        # 买入信号：BIAS突破通道上轨
        buy_condition = self.data['break_upper'] == True
        self.data.loc[buy_condition, 'signal'] = 1
        
        # 卖出信号：BIAS突破通道下轨
        sell_condition = self.data['break_lower'] == True
    ...
    def plot_results(self):
        ...
        plt.title('BIAS通道突破策略回测')
        plt.legend()
        plt.grid(True)
        
        plt.subplot(5, 1, 2)
        plt.plot(self.data.index, self.data['bias_short'], label='BIAS(6日)', color='purple', linewidth=1.5)
        plt.plot(self.data.index, self.data['channel_mid'], label=f'{self.channel_period}日BIAS均线', color='orange', linewidth=1.2)
        plt.plot(self.data.index, self.data['channel_upper'], label='通道上轨', color='green', linestyle='--', linewidth=1)
        plt.plot(self.data.index, self.data['channel_lower'], label='通道下轨', color='red', linestyle='--', linewidth=1)
        plt.fill_between(self.data.index, self.data['channel_lower'], self.data['channel_upper'], 
                        color='gray', alpha=0.1, label='BIAS通道')
        plt.scatter(self.data[self.data['break_upper']].index, self.data[self.data['break_upper']]['bias_short'], 
                   marker='o', color='green', s=50, label='突破上轨')
        plt.scatter(self.data[self.data['break_lower']].index, self.data[self.data['break_lower']]['bias_short'], 
                   marker='o', color='red', s=50, label='突破下轨')
        plt.title('BIAS通道与突破')
        plt.legend()
        plt.grid(True)
        
        plt.subplot(5, 1, 3)
        plt.plot(self.data.index, self.data['bias_position'], label='BIAS相对通道位置', color='brown', linewidth=1.5)
        plt.axhline(y=self.channel_std, color='green', linestyle='--', linewidth=1)
        plt.axhline(y=-self.channel_std, color='red', linestyle='--', linewidth=1)
        plt.title('BIAS相对通道位置')
        plt.legend()
        plt.grid(True)
        
        plt.subplot(5, 1, 4)
        plt.plot(self.data.index, self.data['channel_std'], label='BIAS波动率', color='cyan', linewidth=1.5)
        plt.title('BIAS波动率变化')
        plt.legend()
        plt.grid(True)
        ...
        plt.tight_layout()
        plt.savefig(f"{self.output_dir}/bias_channel_strategy_backtest.png", dpi=300, bbox_inches='tight')
        plt.close()
...
if __name__ == "__main__":
    ...
    # 创建策略实例
    strategy = BiasChannelStrategy(data, short_period=6, channel_period=20, channel_std=2.0)
    
    # 计算指标
    strategy.calculate_bias()
    strategy.calculate_channel()
    ...
    # 统计通道突破情况
    break_upper_count = strategy.data['break_upper'].sum()
    break_lower_count = strategy.data['break_lower'].sum()
    print(f"\n通道突破统计：")
    print(f"突破上轨次数: {break_upper_count}")
    print(f"突破下轨次数: {break_lower_count}")
