...
class BiasAdaptiveStrategy:
    def __init__(self, data, short_period=6, volatility_period=15, base_buy_threshold=-1.5, base_sell_threshold=1.5, stop_loss=0.08, take_profit=0.20):
        self.data = data.copy()
        ...
        self.output_dir = "/Users/quickyang/Documents/workspace/develop/mycode/quant/sharecode/aklean/tech-factor/bias/bias_adaptive"
    ...
    def calculate_volatility(self):
        ...
        # 计算波动率（标准差）
        self.data['volatility'] = self.data['price_change'].rolling(self.volatility_period).std() * np.sqrt(252)
        
        # 计算历史波动率的均值和标准差，用于标准化
        if len(self.data['volatility'].dropna()) >0:
            self.volatility_mean = self.data['volatility'].mean()
            self.volatility_std = self.data['volatility'].std()
        else:
            self.volatility_mean = 0.2
            self.volatility_std = 0.1
        
        # 波动率标准化
        self.data['volatility_standardized'] = (self.data['volatility'] - self.volatility_mean) / self.volatility_std
    ...
    def calculate_adaptive_thresholds(self):
        """计算自适应阈值"""
        # 根据波动率调整阈值
        # 波动率越高，阈值应该越大；波动率越低，阈值应该越小
        self.data['buy_threshold'] = self.base_buy_threshold * (1 + self.data['volatility_standardized'] * 0.5)
        self.data['sell_threshold'] = self.base_sell_threshold * (1 + self.data['volatility_standardized'] * 0.5)
        
        # 设置阈值的上下限，避免极端值
        self.data['buy_threshold'] = self.data['buy_threshold'].clip(-5, -1)
        self.data['sell_threshold'] = self.data['sell_threshold'].clip(1, 5)
    ...
    def generate_signals(self):
        ...
        # 买入信号：BIAS低于动态买入阈值
        buy_condition = self.data['bias_short']< self.data['buy_threshold']
        self.data.loc[buy_condition, 'signal'] = 1
        
        # 卖出信号：BIAS高于动态卖出阈值
        sell_condition = self.data['bias_short'] >self.data['sell_threshold']
    ...
    def plot_results(self):
        ...
        plt.title('BIAS自适应策略回测')
        plt.legend()
        plt.grid(True)
        
        plt.subplot(5, 1, 2)
        plt.plot(self.data.index, self.data['bias_short'], label='BIAS(6日)', color='purple', linewidth=1.5)
        plt.plot(self.data.index, self.data['buy_threshold'], label='动态买入阈值', color='green', linestyle='--', linewidth=1.2)
        plt.plot(self.data.index, self.data['sell_threshold'], label='动态卖出阈值', color='red', linestyle='--', linewidth=1.2)
        plt.title('BIAS指标与动态阈值')
        plt.legend()
        plt.grid(True)
        
        plt.subplot(5, 1, 3)
        plt.plot(self.data.index, self.data['volatility'], label='年化波动率', color='orange', linewidth=1.5)
        plt.title('市场波动率')
        plt.legend()
        plt.grid(True)
        
        plt.subplot(5, 1, 4)
        plt.plot(self.data.index, self.data['volatility_standardized'], label='标准化波动率', color='brown', linewidth=1.5)
        ...
        plt.tight_layout()
        plt.savefig(f"{self.output_dir}/bias_adaptive_strategy_backtest.png", dpi=300, bbox_inches='tight')
        plt.close()
...
if __name__ == "__main__":
    ...
    # 创建策略实例
    strategy = BiasAdaptiveStrategy(data, short_period=6, volatility_period=20)
    ...
    # 统计阈值范围
    buy_threshold_range = f"{strategy.data['buy_threshold'].min():.2f} to {strategy.data['buy_threshold'].max():.2f}"
    sell_threshold_range = f"{strategy.data['sell_threshold'].min():.2f} to {strategy.data['sell_threshold'].max():.2f}"
    print(f"\n阈值范围：")
    print(f"买入阈值范围: {buy_threshold_range}")
    print(f"卖出阈值范围: {sell_threshold_range}")
