...
class AtrDynamicStopStrategy:
    def __init__(self, data, atr_period=14, entry_period=15, stop_loss_multiplier=2.0, trailing_stop_multiplier=1.0, take_profit_multiplier=3.5):
        self.data = data.copy()
        ...
        self.output_dir = "/Users/quickyang/Documents/workspace/develop/mycode/quant/sharecode/aklean/tech-factor/atr/atr_dynamic_stop"
    ...
    def calculate_entry_conditions(self):
        """计算入场条件"""
        # 计算近期最高价和最低价
        self.data['high_rolling'] = self.data['high'].rolling(window=self.entry_period).max()
        self.data['low_rolling'] = self.data['low'].rolling(window=self.entry_period).min()
        
        # 计算突破阈值
        self.data['breakout_high'] = self.data['high_rolling'].shift(1)
        self.data['breakout_low'] = self.data['low_rolling'].shift(1)
    ...
    def generate_signals(self):
        ...
        # 买入信号：价格突破近期高点
        buy_condition = self.data['high'] > self.data['breakout_high']
        self.data.loc[buy_condition, 'signal'] = 1
        
        # 卖出信号：价格突破近期低点
        sell_condition = self.data['low']< self.data['breakout_low']
    ...
    def backtest(self):
        ...
        for i in range(1, len(self.data)):
            ...
            if self.data['signal'].iloc[i] == 1 and position == 0:
                ...
                # 设置初始止损和止盈
                initial_stop_loss = entry_price - self.data['atr'].iloc[i] * self.stop_loss_multiplier
                take_profit_price = entry_price + self.data['atr'].iloc[i] * self.take_profit_multiplier
                
                self.data.at[self.data.index[i], 'position'] = position
                self.data.at[self.data.index[i], 'entry_price'] = entry_price
                self.data.at[self.data.index[i], 'stop_loss'] = initial_stop_loss
            ...
            # 更新跟踪止损
                ...
                # 计算新的跟踪止损
                new_trailing_stop = self.data['close'].iloc[i] - self.data['atr'].iloc[i] * self.trailing_stop_multiplier
                # 如果新止损更高，则更新
                trailing_stop = max(trailing_stop, new_trailing_stop)
                
                self.data.at[self.data.index[i], 'trailing_stop'] = trailing_stop
                self.data.at[self.data.index[i], 'position'] = position
                self.data.at[self.data.index[i], 'entry_price'] = entry_price
            ...
            # 止盈触发
            elif position == 1 and self.data['high'].iloc[i] >= self.data['take_profit'].iloc[i-1]:
                position = 0
                exit_price = self.data['take_profit'].iloc[i-1]
                self.data.at[self.data.index[i], 'strategy_returns'] = (exit_price - entry_price) / entry_price
    ...
    def plot_results(self):
        ...
        for i in range(len(self.data)):
            if self.data['position'].iloc[i] == 1:
                if not in_position:
                    in_position = True
                    stop_losses.append((self.data.index[i], self.data['trailing_stop'].iloc[i]))
                else:
                    stop_losses.append((self.data.index[i], self.data['trailing_stop'].iloc[i]))
            else:
                if in_position:
                    ...
                    if stop_losses:
                        x, y = zip(*stop_losses)
                        plt.plot(x, y, color='orange', linestyle='--', alpha=0.7, label='跟踪止损' if '跟踪止损' not in plt.gca().get_legend_handles_labels()[1] else "")
                    stop_losses = []
        
        plt.title('ATR动态止损策略 - 价格与止损')
        plt.legend()
        plt.grid(True)
        ...
        plt.title('ATR动态止损策略 - 累计收益')
        plt.legend()
        plt.grid(True)
        
        plt.tight_layout()
        plt.savefig(f"{self.output_dir}/atr_dynamic_stop_strategy_backtest.png", dpi=300, bbox_inches='tight')
        plt.close()
    
    def run_strategy(self):
        """运行完整策略"""
        self.calculate_atr()
        self.calculate_entry_conditions()
        ...
        # 保存回测数据
        self.data.to_csv(f"{self.output_dir}/atr_dynamic_stop_strategy_backtest.csv")
...
if __name__ == "__main__":
    ...
    # 运行策略
    strategy = AtrDynamicStopStrategy(data)
