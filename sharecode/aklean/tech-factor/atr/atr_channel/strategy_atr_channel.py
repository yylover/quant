...
class AtrChannelStrategy:
    def __init__(self, data, atr_period=14, channel_period=15, channel_multiplier=0.8, stop_loss=0.08, take_profit=0.20):
        self.data = data.copy()
        ...
        self.output_dir = "/Users/quickyang/Documents/workspace/develop/mycode/quant/sharecode/aklean/tech-factor/atr/atr_channel"
    ...
    def calculate_channel(self):
        """计算ATR通道"""
        # 计算通道中轨（通常使用移动平均）
        self.data['channel_mid'] = self.data['close'].rolling(window=self.channel_period).mean()
        
        # 计算通道上轨和下轨
        self.data['channel_high'] = self.data['channel_mid'] + self.data['atr'] * self.channel_multiplier
        self.data['channel_low'] = self.data['channel_mid'] - self.data['atr'] * self.channel_multiplier
    ...
    def generate_signals(self):
        ...
        # 买入信号：价格突破通道上轨
        buy_condition = self.data['close'] > self.data['channel_high'].shift(1)
        self.data.loc[buy_condition, 'signal'] = 1
        
        # 卖出信号：价格突破通道下轨
        sell_condition = self.data['close']< self.data['channel_low'].shift(1)
    ...
    def plot_results(self):
        ...
        plt.plot(self.data['channel_mid'], label='通道中轨', color='blue', linestyle='--', alpha=0.7)
        plt.plot(self.data['channel_high'], label='通道上轨', color='green', linestyle='--', alpha=0.7)
        plt.plot(self.data['channel_low'], label='通道下轨', color='red', linestyle='--', alpha=0.7)
        ...
        plt.title('ATR通道策略 - 累计收益')
        plt.legend()
        plt.grid(True)
        
        plt.tight_layout()
        plt.savefig(f"{self.output_dir}/atr_channel_strategy_backtest.png", dpi=300, bbox_inches='tight')
        plt.close()
    
    def run_strategy(self):
        """运行完整策略"""
        self.calculate_atr()
        self.calculate_channel()
        ...
        # 保存回测数据
        self.data.to_csv(f"{self.output_dir}/atr_channel_strategy_backtest.csv")
...
if __name__ == "__main__":
    ...
    # 运行策略
    strategy = AtrChannelStrategy(data)
