...
class AtrTrendFollowingStrategy:
    def __init__(self, data, atr_period=14, ma_period=30, trend_threshold=0.3, stop_loss=0.08, take_profit=0.20):
        self.data = data.copy()
        ...
        self.output_dir = "/Users/quickyang/Documents/workspace/develop/mycode/quant/sharecode/aklean/tech-factor/atr/atr_trend_following"
    ...
    def calculate_trend_indicators(self):
        ...
        # 计算价格相对于均线的偏离（标准化）
        self.data['price_deviation'] = (self.data['close'] - self.data['ma']) / self.data['atr']
        
        # 计算趋势强度
        self.data['trend_strength'] = abs(self.data['price_deviation'])
    ...
    def generate_signals(self):
        ...
        # 买入信号：价格在均线上方且趋势强度超过阈值
        buy_condition = (self.data['close'] > self.data['ma']) & (self.data['price_deviation'] > self.trend_threshold)
        self.data.loc[buy_condition, 'signal'] = 1
        
        # 卖出信号：价格在均线下方且趋势强度超过阈值
        sell_condition = (self.data['close']< self.data['ma']) & (self.data['price_deviation'] < -self.trend_threshold)
    ...
    def backtest(self):
        ...
        for i in range(1, len(self.data)):
            ...
            if self.data['signal'].iloc[i] == 1 and position == 0:
                ...
                # 设置止损和止盈
                stop_loss_price = entry_price - self.data['atr'].iloc[i] * 3.0
                take_profit_price = entry_price + self.data['atr'].iloc[i] * 4.0
    ...
    def plot_results(self):
        ...
        plt.title('ATR趋势跟踪策略 - 累计收益')
        plt.legend()
        plt.grid(True)
        
        plt.tight_layout()
        plt.savefig(f"{self.output_dir}/atr_trend_following_strategy_backtest.png", dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def run_strategy(self):
        ...
        # 保存回测数据
        self.data.to_csv(f"{self.output_dir}/atr_trend_following_strategy_backtest.csv")
...
if __name__ == "__main__":
    ...
    # 运行策略
    strategy = AtrTrendFollowingStrategy(data)
