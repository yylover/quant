...
class AtrMeanReversionStrategy:
    def __init__(self, data, atr_period=14, reversion_period=20, reversion_threshold=1.0, stop_loss=0.08, take_profit=0.20):
        self.data = data.copy()
        ...
        self.output_dir = "/Users/quickyang/Documents/workspace/develop/mycode/quant/sharecode/aklean/tech-factor/atr/atr_mean_reversion"
    ...
    def calculate_reversion_indicators(self):
        """计算均值回归指标"""
        # 计算移动平均线作为均值
        self.data['ma'] = self.data['close'].rolling(window=self.reversion_period).mean()
        
        # 计算价格相对于均线的偏离（标准化）
        self.data['deviation'] = (self.data['close'] - self.data['ma']) / self.data['atr']
        
        # 计算偏离百分比
        self.data['deviation_pct'] = (self.data['close'] - self.data['ma']) / self.data['ma'] * 100
    ...
    def generate_signals(self):
        ...
        # 买入信号：价格偏离均线下方超过阈值（超卖）
        buy_condition = self.data['deviation']< -self.reversion_threshold
        self.data.loc[buy_condition, 'signal'] = 1
        
        # 卖出信号：价格偏离均线上方超过阈值（超买）
        sell_condition = self.data['deviation'] >self.reversion_threshold
    ...
    def plot_results(self):
        ...
        plt.plot(self.data['ma'], label='移动平均线', color='blue', linestyle='--', alpha=0.7)
        
        # 计算偏离带
        upper_band = self.data['ma'] + self.data['atr'] * self.reversion_threshold
        lower_band = self.data['ma'] - self.data['atr'] * self.reversion_threshold
        plt.plot(upper_band, label='偏离上限', color='red', linestyle='--', alpha=0.5)
        plt.plot(lower_band, label='偏离下限', color='green', linestyle='--', alpha=0.5)
        ...
        plt.title('ATR均值回归策略 - 累计收益')
        plt.legend()
        plt.grid(True)
        
        plt.tight_layout()
        plt.savefig(f"{self.output_dir}/atr_mean_reversion_strategy_backtest.png", dpi=300, bbox_inches='tight')
        plt.close()
    
    def run_strategy(self):
        """运行完整策略"""
        self.calculate_atr()
        self.calculate_reversion_indicators()
        ...
        # 保存回测数据
        self.data.to_csv(f"{self.output_dir}/atr_mean_reversion_strategy_backtest.csv")
...
if __name__ == "__main__":
    ...
    # 运行策略
    strategy = AtrMeanReversionStrategy(data)
