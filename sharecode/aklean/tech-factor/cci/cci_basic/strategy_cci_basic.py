...
class CCIBasicStrategy:
    """CCI基础超买超卖策略"""
    
    def __init__(self, data, cci_period=14, overbought_threshold=150, oversold_threshold=-150):
        """
        初始化CCI基础策略
        ...
        self.output_dir = "/Users/quickyang/Documents/workspace/develop/mycode/quant/sharecode/aklean/tech-factor/cci/cci_basic"
    ...
    def generate_signals(self):
        ...
        # 买入信号：CCI从超卖区域(-100以下)上穿
        buy_condition = (self.data['CCI'] > self.oversold_threshold) & (self.data['CCI'].shift(1)<= self.oversold_threshold)
        self.data.loc[buy_condition, 'signal'] = 1
        
        # 卖出信号：CCI从超买区域(100以上)下穿
        sell_condition = (self.data['CCI'] < self.overbought_threshold) & (self.data['CCI'].shift(1) >= self.overbought_threshold)
    ...
    def calculate_metrics(self):
        ...
        metrics = {
            ...
            '参数': f"cci_period={self.cci_period}, overbought={self.overbought_threshold}, oversold={self.oversold_threshold}"
        }
    ...
    def plot_backtest(self):
        ...
        plt.axhline(y=self.overbought_threshold, color='red', linestyle='--', alpha=0.7, label=f'超买阈值({self.overbought_threshold})')
        plt.axhline(y=self.oversold_threshold, color='green', linestyle='--', alpha=0.7, label=f'超卖阈值({self.oversold_threshold})')
        plt.axhline(y=0, color='gray', linestyle='-', alpha=0.5)
        
        plt.fill_between(self.data.index, self.data['CCI'], self.overbought_threshold, where=self.data['CCI']> self.overbought_threshold, color='red', alpha=0.2)
        plt.fill_between(self.data.index, self.data['CCI'], self.oversold_threshold, where=self.data['CCI']< self.oversold_threshold, color='green', alpha=0.2)
        ...
        plt.tight_layout()
        plt.savefig(f"{self.output_dir}/cci_basic_strategy_backtest.png", dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def run_strategy(self):
        ...
        # 保存回测数据
        self.data.to_csv(f"{self.output_dir}/cci_basic_strategy_backtest.csv")
        
        # 保存策略说明
        with open(f"{self.output_dir}/cci_basic_strategy说明.md", 'w', encoding='utf-8') as f:
            f.write(f"""# CCI基础超买超卖策略说明
...
- 超买阈值: {self.overbought_threshold}
