...
class RSITrendFollowingSystem:
    """RSI趋势跟踪交易系统"""
    
    def __init__(self, data, rsi_period=14, ma_window=50):
        """初始化交易系统"""
        self.data = data.copy()
        self.rsi_period = rsi_period          # RSI周期：14天
        self.ma_window = ma_window            # 均线周期：50天
    ...
    def calculate_indicators(self):
        ...
        # 计算RSI的移动平均，用于平滑RSI信号
        self.data['rsi_ma'] = self.data['rsi'].rolling(window=10).mean()
    ...
    def generate_signals(self):
        ...
        for i in range(1, len(self.data)):
            # 买入信号：
            # 条件1：价格在均线上方（趋势向上）
            # 条件2：RSI从低位（<50）向上穿越50
            if (self.data['price_above_ma'].iloc[i] and
                self.data['rsi'].iloc[i] >= 50 and 
                self.data['rsi'].iloc[i-1]< 50):
                self.data.loc[self.data.index[i], 'signal'] = 1
            
            # 卖出信号：
            # 条件1：价格在均线下方（趋势向下）
            # 条件2：RSI从高位（>50）向下穿越50
            elif (not self.data['price_above_ma'].iloc[i] and
                  self.data['rsi'].iloc[i]<= 50 and 
                  self.data['rsi'].iloc[i-1] > 50):
                self.data.loc[self.data.index[i], 'signal'] = -1
    ...
    def plot_results(self):
        ...
        plt.plot(self.data.index, self.data['rsi_ma'], label='RSI均线', color='orange', linestyle='--')
        plt.axhline(y=50, color='gray', linestyle='--', label='中性线(50)')
        plt.ylabel('RSI')
        plt.legend()
        plt.grid(True)
        ...
        plt.savefig(f"{output_dir}/rsi_trend_following_backtest.png", dpi=300, bbox_inches='tight')
        plt.close()
        print("回测图表已保存")
    ...
    def save_results(self):
        ...
        # 保存回测数据
        self.data.to_csv(f"{output_dir}/rsi_trend_following_backtest.csv")
        print("回测数据已保存")
        
        # 保存策略说明和回测结果
        with open(f"{output_dir}/rsi_trend_following说明.md", 'w', encoding='utf-8') as f:
            f.write("# RSI趋势跟踪交易系统\n\n")
            f.write("## 策略说明\n\n")
            f.write("### 核心思路\n")
            f.write("结合RSI指标和均线指标，在趋势确认的基础上利用RSI的中性线穿越进行交易。\n\n")
            f.write("### 系统结构\n")
            f.write(f"- **RSI周期**: {self.rsi_period}天\n")
            f.write(f"- **均线周期**: {self.ma_window}天\n\n")
            f.write("### 交易逻辑\n")
            f.write("1. **买入条件**: 价格在均线上方且RSI从下方穿越50\n")
            f.write("2. **卖出条件**: 价格在均线下方且RSI从上方穿越50\n\n")
            f.write("## 回测结果\n\n")
            f.write("### 性能指标\n")
            total_return = (self.data['cumulative_strategy'].iloc[-1] - 1) * 100
            benchmark_return = (self.data['cumulative_benchmark'].iloc[-1] - 1) * 100
            ...
            daily_returns = self.data['strategy_return'].dropna()
            sharpe_ratio = (daily_returns.mean() * 365 - 0.03) / (daily_returns.std() * np.sqrt(365)) if (len(daily_returns) >0 and daily_returns.std() > 0) else 0
...
if __name__ == "__main__":
    ...
    # 创建并运行RSI趋势跟踪交易系统
    rsi_system = RSITrendFollowingSystem(data)
