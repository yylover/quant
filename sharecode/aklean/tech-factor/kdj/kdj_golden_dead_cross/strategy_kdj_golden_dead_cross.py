...
class KDJGoldenDeadCrossStrategy:
    def __init__(self, data):
        self.data = data.copy()
        self.output_dir = "/Users/quickyang/Documents/workspace/develop/mycode/quant/sharecode/aklean/tech-factor/kdj/kdj_golden_dead_cross"
    ...
    def generate_signals(self):
        ...
        for i in range(1, len(self.data)):
            # 金叉买入信号：K线上穿D线
            if self.data['k'].iloc[i] > self.data['d'].iloc[i] and self.data['k'].iloc[i-1] <= self.data['d'].iloc[i-1]:
                self.data.loc[self.data.index[i], 'signal'] = 1
            
            # 死叉卖出信号：K线下穿D线
            elif self.data['k'].iloc[i]< self.data['d'].iloc[i] and self.data['k'].iloc[i-1] >= self.data['d'].iloc[i-1]:
                self.data.loc[self.data.index[i], 'signal'] = -1
    ...
    def plot_results(self):
        ...
        plt.title('KDJ金叉死叉策略回测 - 价格与交易信号', fontsize=14)
        ...
        plt.tight_layout()
        plt.savefig(f"{self.output_dir}/kdj_golden_dead_cross_backtest.png", dpi=300, bbox_inches='tight')
        plt.close()
    
    def save_results(self):
        """保存回测结果"""
        self.data.to_csv(f"{self.output_dir}/kdj_golden_dead_cross_backtest.csv")
...
"""
    ...
    output_dir = "/Users/quickyang/Documents/workspace/develop/mycode/quant/sharecode/aklean/tech-factor/kdj/kdj_golden_dead_cross"
    os.makedirs(output_dir, exist_ok=True)
    
    with open(f"{output_dir}/kdj_golden_dead_cross说明.md", 'w', encoding='utf-8') as f:
        f.write(description)
...
if __name__ == "__main__":
    ...
    # 创建策略实例
    strategy = KDJGoldenDeadCrossStrategy(data)
