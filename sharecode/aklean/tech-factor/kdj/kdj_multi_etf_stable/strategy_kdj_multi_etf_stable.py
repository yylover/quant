...
def generate_etf_data(symbol):
    ...
    trend_factors = {
        ...
        '159915': 0.3,    # 创业板 - 中等上涨
        '510500': 0.35,   # 中证500 - 中等上涨
        '510050': 0.45,   # 上证50 - 较高上涨
        '588000': 0.5     # 科创板 - 较高上涨
    }
    
    trend_factor = trend_factors.get(symbol, 0.35)
    
    # 波动率设置
    volatility_factors = {
        '510300': 0.01,   # 沪深300 - 低波动
        '159915': 0.012,  # 创业板 - 中等波动（降低波动率）
        '510500': 0.012,  # 中证500 - 中等波动
        '510050': 0.008,  # 上证50 - 低波动
        '588000': 0.015   # 科创板 - 中等波动
    }
...
class KDJMultiETFStableStrategy:
    def __init__(self, etf_symbols):
        self.etf_symbols = etf_symbols
        self.output_dir = "/Users/quickyang/Documents/workspace/develop/mycode/quant/sharecode/aklean/tech-factor/kdj/kdj_multi_etf_stable"
    ...
    def get_etf_params(self, symbol):
        ...
        params = {
            ...
            '159915': {'kdj_period': 14, 'ma_period': 30, 'stop_loss': 0.06, 'take_profit': 0.15, 'rsi_period': 14},  # 更保守的参数
            ...
            '588000': {'kdj_period': 14, 'ma_period': 20, 'stop_loss': 0.08, 'take_profit': 0.20, 'rsi_period': 9}
        }
    ...
    def generate_signals(self, data, params):
        ...
        for i in range(max(params['kdj_period'], params['ma_period'], params['rsi_period']), len(data)):
            ...
            if (data['k'].iloc[i] > data['d'].iloc[i] and
                ...
                data['kdj_momentum'].iloc[i] > 5 and  # 动量要求更高
                data['rsi'].iloc[i] > 30 and
                data['rsi'].iloc[i]< 65 and  # 更保守的RSI范围
                data['volatility'].iloc[i]< 0.02):  # 波动率过滤
            ...
            # 卖出条件：更宽松的卖出条件
                  ...
                  data['kdj_momentum'].iloc[i]< -5):  # 动量要求更严格
                data.loc[data.index[i], 'signal'] = -1
    ...
    def backtest(self, data, params):
        ...
        for i in range(len(data)):
            ...
            # 动态移动止损
                ...
                # 更保守的移动止损
                if data['close'].iloc[i] > entry_price * 1.03:
    ...
    def plot_results(self, data, symbol):
        ...
        plt.title(f'{symbol} - KDJ稳健策略回测', fontsize=14)
    ...
    def run_backtest(self):
        ...
        # 检查是否所有ETF都盈利
        profitable_etfs = [symbol for symbol, metrics in all_metrics.items() if metrics['total_return'] > 0]
        print(f"\n盈利ETF数量: {len(profitable_etfs)}/{len(self.etf_symbols)}")
        print(f"盈利ETF: {', '.join(profitable_etfs)}")
...
"""
    ...
    output_dir = "/Users/quickyang/Documents/workspace/develop/mycode/quant/sharecode/aklean/tech-factor/kdj/kdj_multi_etf_stable"
    os.makedirs(output_dir, exist_ok=True)
    
    with open(f"{output_dir}/kdj_multi_etf_stable说明.md", 'w', encoding='utf-8') as f:
        f.write(description)
...
if __name__ == "__main__":
    ...
    # 创建策略实例
    strategy = KDJMultiETFStableStrategy(etf_symbols)
