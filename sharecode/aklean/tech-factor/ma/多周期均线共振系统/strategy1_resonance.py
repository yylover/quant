...
class MultiPeriodResonanceSystem:
    """多周期均线共振交易系统"""
    
    def __init__(self, data, fast_window=5, medium_window=20, slow_window=60, ultra_slow_window=120):
        """初始化交易系统"""
        self.data = data.copy()
        self.fast_window = fast_window        # 快线：5日（短期趋势）
        self.medium_window = medium_window    # 中线：20日（中期趋势）
        self.slow_window = slow_window        # 慢线：60日（长期趋势）
        self.ultra_slow_window = ultra_slow_window  # 超慢线：120日（大趋势）
    ...
    def calculate_ma(self):
        ...
        # 计算超慢线
        self.data['ultra_slow_ma'] = self.data['收盘'].rolling(window=self.ultra_slow_window).mean()
        
        # 计算均线斜率（判断趋势方向）
        self.data['ultra_slow_slope'] = self.data['ultra_slow_ma'].diff(20) / 20
        ...
        # 价格与均线的关系
        self.data['price_above_all'] = (self.data['收盘'] > self.data['fast_ma']) & \
                                     (self.data['收盘'] > self.data['medium_ma']) & \
                                     (self.data['收盘'] > self.data['slow_ma']) & \
                                     (self.data['收盘'] > self.data['ultra_slow_ma'])
    ...
    def generate_signals(self):
        ...
        for i in range(1, len(self.data)):
            ...
            # 4. 快线上穿中线（短期买入信号）
            # 5. 价格站在所有均线上方
            if (self.data['ultra_slow_slope'].iloc[i] > 0 and
                self.data['slow_slope'].iloc[i] > 0 and
                self.data['medium_slow_diff'].iloc[i] > 0 and
                self.data['medium_slow_diff'].iloc[i-1] <= 0 and
                self.data['fast_medium_diff'].iloc[i] > 0 and
                self.data['fast_medium_diff'].iloc[i-1] <= 0 and
                self.data['price_above_all'].iloc[i]):
            ...
            # 3. 慢线向下（长期趋势转弱）
                 ...
                 (self.data['收盘'].iloc[i] < self.data['medium_ma'].iloc[i]) or \
                 (self.data['slow_slope'].iloc[i]< 0):
                self.data.loc[self.data.index[i], 'signal'] = -1
    
    def backtest(self):
        """执行回测"""
        print(f"开始回测...")
        print(f"快线: {self.fast_window}天, 中线: {self.medium_window}天")
        print(f"慢线: {self.slow_window}天, 超慢线: {self.ultra_slow_window}天")
    ...
    def plot_results(self):
        ...
        plt.plot(self.data.index, self.data['fast_ma'], label=f'{self.fast_window}日均线', color='#ff7f0e', linewidth=1.2)
        ...
        plt.plot(self.data.index, self.data['ultra_slow_ma'], label=f'{self.ultra_slow_window}日均线', color='#d62728', linewidth=1.2)
        ...
        plt.title('沪深300指数 - 多周期均线共振系统', fontsize=16, fontweight='bold')
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(BASE_DIR, 'strategy1_backtest.png'), dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def run(self):
        ...
        # 保存回测数据
        self.data.to_csv(os.path.join(BASE_DIR, 'strategy1_backtest.csv'))
        print("\n回测数据已保存")
    ...
    def save_strategy_report(self, performance):
        """保存策略说明和回测结果"""
        report_content = f"""# 多周期均线共振交易系统（策略1）
...
"""
        ...
        with open(os.path.join(BASE_DIR, 'strategy1说明.md'), 'w', encoding='utf-8') as f:
...
def main():
    ...
    try:
        ...
        # 运行策略回测
        system = MultiPeriodResonanceSystem(data.copy())
        system.run()
