...
class SlopeFilterSystem:
    """均线斜率过滤交易系统"""
    
    def __init__(self, data, ma_window=20, slope_period=10, slope_threshold=0.2):
        """初始化交易系统"""
        self.data = data.copy()
        self.ma_window = ma_window              # 均线周期：20日
        self.slope_period = slope_period        # 斜率计算周期：10天
        self.slope_threshold = slope_threshold  # 斜率阈值：0.2
    ...
    def generate_signals(self):
        ...
        for i in range(1, len(self.data)):
            ...
            # 1. 均线斜率 > 阈值（趋势强劲向上）
            # 2. 价格在均线上方
            if (self.data['ma_slope'].iloc[i] > self.slope_threshold and
            ...
            # 1. 均线斜率< -阈值（趋势反转向下）
            # 2. 价格跌破均线
            elif (self.data['ma_slope'].iloc[i]< -self.slope_threshold or
    ...
    def backtest(self):
        """执行回测"""
        print(f"开始回测...")
        print(f"均线周期: {self.ma_window}天, 斜率周期: {self.slope_period}天, 斜率阈值: {self.slope_threshold}")
    ...
    def plot_results(self):
        ...
        plt.title('沪深300指数 - 均线斜率过滤系统', fontsize=16, fontweight='bold')
        ...
        plt.axhline(y=self.slope_threshold, color='green', linestyle='--', label=f'斜率阈值({self.slope_threshold})')
        plt.axhline(y=-self.slope_threshold, color='red', linestyle='--', label=f'斜率阈值(-{self.slope_threshold})')
        plt.axhline(y=0, color='blue', linestyle='--', label='零轴')
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(BASE_DIR, 'slope_filter_backtest.png'), dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def run(self):
        ...
        # 保存回测数据
        self.data.to_csv(os.path.join(BASE_DIR, 'slope_filter_backtest.csv'))
        print("\n回测数据已保存")
...
- **斜率周期**：{self.slope_period}天（计算均线斜率）
- **斜率阈值**：±{self.slope_threshold}

### 开仓条件（必须同时满足）
1. **均线斜率 > {self.slope_threshold}**：均线斜率大于阈值，表明趋势强劲向上
2. **价格在均线上方**：价格位于均线上方，确认多头趋势

### 平仓条件（满足任一即可）
1. **均线斜率< -{self.slope_threshold}**：均线斜率小于负阈值，表明趋势反转向下
...
"""
        ...
        with open(os.path.join(BASE_DIR, 'slope_filter说明.md'), 'w', encoding='utf-8') as f:
...
def main():
    ...
    try:
        ...
        # 运行策略回测
        system = SlopeFilterSystem(data.copy())
        system.run()
