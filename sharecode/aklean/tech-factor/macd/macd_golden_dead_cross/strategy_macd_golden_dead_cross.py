...
class MACDGoldenDeadCrossStrategy:
    ...
    def generate_signals(self):
        ...
        for i in range(1, len(self.data)):
            # 金叉买入信号：DIF上穿DEA
            if self.data['dif'].iloc[i] > self.data['dea'].iloc[i] and self.data['dif'].iloc[i-1] <= self.data['dea'].iloc[i-1]:
                self.data.loc[self.data.index[i], 'signal'] = 1
            
            # 死叉卖出信号：DIF下穿DEA
            elif self.data['dif'].iloc[i]< self.data['dea'].iloc[i] and self.data['dif'].iloc[i-1] >=self.data['dea'].iloc[i-1]:
                self.data.loc[self.data.index[i], 'signal'] = -1
    ...
    def plot_results(self):
        ...
        plt.title('MACD金叉死叉策略回测结果', fontsize=14, fontweight='bold')
        ...
        # 保存图表
        output_dir = "/Users/quickyang/Documents/workspace/develop/mycode/quant/sharecode/aklean/tech-factor/macd/macd_golden_dead_cross"
        os.makedirs(output_dir, exist_ok=True)
        
        plt.savefig(f"{output_dir}/macd_golden_dead_cross_backtest.png", dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def run_backtest(self):
        ...
        # 保存回测数据
        output_dir = "/Users/quickyang/Documents/workspace/develop/mycode/quant/sharecode/aklean/tech-factor/macd/macd_golden_dead_cross"
        self.data.to_csv(f"{output_dir}/macd_golden_dead_cross_backtest.csv")
...
if __name__ == "__main__":
    ...
    # 创建策略实例
    strategy = MACDGoldenDeadCrossStrategy(hs300_data)
    
    # 运行回测
    metrics = strategy.run_backtest()
    
    # 输出结果
    print("=== MACD金叉死叉策略回测结果 ===")
