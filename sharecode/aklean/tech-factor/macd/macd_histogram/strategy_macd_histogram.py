...
class MACDHistogramStrategy:
    ...
    def calculate_macd(self):
        ...
        # 计算柱状图变化
        self.data['macd_hist_change'] = self.data['macd_hist'].diff()
    ...
    def generate_signals(self):
        ...
        for i in range(1, len(self.data)):
            # 买入信号：MACD柱由负转正
            if self.data['macd_hist'].iloc[i] > 0 and self.data['macd_hist'].iloc[i-1] <= 0:
                self.data.loc[self.data.index[i], 'signal'] = 1
            
            # 卖出信号：MACD柱由正转负
            elif self.data['macd_hist'].iloc[i]< 0 and self.data['macd_hist'].iloc[i-1] >=0:
                self.data.loc[self.data.index[i], 'signal'] = -1
    ...
    def plot_results(self):
        ...
        plt.title('MACD柱状图策略回测结果', fontsize=14, fontweight='bold')
        ...
        plt.subplot(3, 1, 2)
        # 使用不同颜色区分正负柱状图
        pos_hist = self.data[self.data['macd_hist'] >= 0]['macd_hist']
        neg_hist = self.data[self.data['macd_hist']< 0]['macd_hist']
        
        plt.bar(pos_hist.index, pos_hist, color='red', alpha=0.5, label='正MACD柱')
        plt.bar(neg_hist.index, neg_hist, color='green', alpha=0.5, label='负MACD柱')
        plt.axhline(y=0, color='black', linestyle='-', alpha=0.3)
        plt.ylabel('MACD柱', fontsize=12)
        plt.grid(True, alpha=0.3)
        plt.legend()
        ...
        # 保存图表
        output_dir = "/Users/quickyang/Documents/workspace/develop/mycode/quant/sharecode/aklean/tech-factor/macd/macd_histogram"
        os.makedirs(output_dir, exist_ok=True)
        
        plt.savefig(f"{output_dir}/macd_histogram_backtest.png", dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def run_backtest(self):
        ...
        # 保存回测数据
        output_dir = "/Users/quickyang/Documents/workspace/develop/mycode/quant/sharecode/aklean/tech-factor/macd/macd_histogram"
        self.data.to_csv(f"{output_dir}/macd_histogram_backtest.csv")
...
if __name__ == "__main__":
    ...
    # 创建策略实例
    strategy = MACDHistogramStrategy(hs300_data)
