...
class ADXTrendFollowingStrategy:
    ...
    def generate_signals(self):
        ...
        # 趋势跟随策略：ADX > 20 表示趋势存在
        # +DI > -DI 表示上升趋势，-DI > +DI 表示下降趋势
        
        # 买入信号：ADX > 20，+DI > -DI，价格在均线上方
        buy_condition = (
            (data['adx'] > 20) &
    ...
    def plot_results(self):
        ...
        ax1.set_title('ADX趋势跟随策略 - 沪深300', fontsize=16)
        ...
        plt.tight_layout()
        plt.savefig('adx_trend_following_strategy_backtest.png', dpi=300, bbox_inches='tight')
    
    def save_results(self):
        """保存回测结果"""
        # 保存交易记录
        trades = self.data[self.data['signal'] != 0].copy()
        trades.to_csv('adx_trend_following_strategy_backtest.csv')
    
    def generate_report(self):
        """生成策略说明文档"""
        report = f"""# ADX趋势跟随策略说明
...
"""
        ...
        with open('adx_trend_following_strategy说明.md', 'w', encoding='utf-8') as f:
            f.write(report)
...
if __name__ == '__main__':
    strategy = ADXTrendFollowingStrategy()
    strategy.run()
