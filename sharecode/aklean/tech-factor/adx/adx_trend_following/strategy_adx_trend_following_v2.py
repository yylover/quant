...
class ADXTrendFollowingStrategyV2:
    def __init__(self):
        self.data = None
        self.results = {}
    ...
    def calculate_adx(self, period=10):
        """计算ADX指标，使用更短的周期"""
        data = self.data.copy()
    ...
    def calculate_ma(self, ma_period=10):
        """计算更短周期的移动平均线"""
        data = self.data.copy()
    ...
    def generate_signals(self):
        ...
        # 优化策略：降低ADX阈值，使用更灵敏的均线
        # 买入信号：ADX > 15，+DI > -DI，价格在均线上方
        buy_condition = (
            (data['adx'] > 15) &
    ...
    def plot_results(self):
        ...
        ax1.plot(data.index, data['ma'], label='10日均线', color='orange', alpha=0.7)
        ...
        ax1.set_title('ADX趋势跟随策略V2 - 沪深300', fontsize=16)
        ...
        # 添加ADX阈值线
        ax2.axhline(y=15, color='gray', linestyle='--', label='ADX阈值(15)')
        ...
        plt.tight_layout()
        plt.savefig('adx_trend_following_strategy_backtest_v2.png', dpi=300, bbox_inches='tight')
    
    def save_results(self):
        """保存回测结果"""
        # 保存交易记录
        trades = self.data[self.data['signal'] != 0].copy()
        trades.to_csv('adx_trend_following_strategy_backtest_v2.csv')
    
    def generate_report(self):
        """生成策略说明文档"""
        report = f"""# ADX趋势跟随策略V2说明
...
"""
        ...
        with open('adx_trend_following_strategy说明_v2.md', 'w', encoding='utf-8') as f:
            f.write(report)
...
if __name__ == '__main__':
    strategy = ADXTrendFollowingStrategyV2()
    strategy.run()
