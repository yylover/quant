...
class ADXOptimizedStrategyV2:
    ...
    def generate_signals(self):
        ...
        # 全新策略设计：
        # 1. 趋势确认：使用长期均线确认大趋势
        # 2. 入场时机：使用短期ADX信号确认入场点
        # 3. 持仓逻辑：在大趋势确定的情况下，尽可能保持持仓
    ...
    def plot_results(self):
        ...
        ax1.set_title('ADX优化策略V2 - 沪深300', fontsize=16)
        ...
        # 添加ADX阈值线
        ax2.axhline(y=18, color='gray', linestyle='--', label='ADX阈值(18)')
        ...
        plt.tight_layout()
        plt.savefig('adx_optimized_strategy_backtest_v2.png', dpi=300, bbox_inches='tight')
    
    def save_results(self):
        """保存回测结果"""
        # 保存交易记录
        trades = self.data[self.data['signal'] != 0].copy()
        trades.to_csv('adx_optimized_strategy_backtest_v2.csv')
    
    def generate_report(self):
        """生成策略说明文档"""
        report = f"""# ADX优化策略V2说明
...
"""
        ...
        with open('adx_optimized_strategy说明_v2.md', 'w', encoding='utf-8') as f:
            f.write(report)
...
if __name__ == '__main__':
    strategy = ADXOptimizedStrategyV2()
    strategy.run()
