...
class StochasticCrossoverStrategy:
    def __init__(self):
        self.data = None
    ...
    def generate_signals(self):
        ...
        # 避免连续信号
        self.data['golden_cross'] = self.data['golden_cross'] & ~self.data['golden_cross'].shift(1).fillna(False)
        self.data['death_cross'] = self.data['death_cross'] & ~self.data['death_cross'].shift(1).fillna(False)
    ...
    def backtest(self):
        ...
        for i in range(1, len(self.data)):
            # 买入信号：黄金交叉
            if self.data['golden_cross'].iloc[i] and position == 0:
                position = 1
            
            # 卖出信号：死亡交叉
            elif self.data['death_cross'].iloc[i] and position == 1:
                position = 0
    ...
    def plot_results(self):
        ...
        # 买卖信号
        buy_signals = self.data[self.data['golden_cross']]
        sell_signals = self.data[self.data['death_cross']]
        ...
        ax1.set_title('Stochastic交叉策略回测', fontsize=16)
        ...
        plt.tight_layout()
        plt.savefig('stochastic_crossover_strategy_backtest.png', dpi=300, bbox_inches='tight')
    ...
    def save_trades(self):
        ...
        trades_df.to_csv('stochastic_crossover_strategy_backtest.csv', index=False)
...
## 交易统计
- **总交易次数**：{self.data['golden_cross'].sum() + self.data['death_cross'].sum()}次
- **买入次数**：{self.data['golden_cross'].sum()}次
- **卖出次数**：{self.data['death_cross'].sum()}次
...
"""
        ...
        with open('stochastic_crossover_strategy说明.md', 'w', encoding='utf-8') as f:
            f.write(doc)
...
if __name__ == '__main__':
    strategy = StochasticCrossoverStrategy()
    strategy.run()
